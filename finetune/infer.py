...
The name of the target monster (e.g., "Deathclaw Coyotes")
Exactly N stories (N = 1, 2, or 3), clearly labeled as "Story 1", "Story 2", etc.
All stories together form one cohesive narrative universe. You must combine clues from all stories to answer.
...
Use ONLY information directly stated or logically inferable about the target monster across all provided stories.
Never guess. If an attribute is not mentioned, use the most neutral/default value:
For lists: use []
...
Do not confuse attributes of other monsters-even if they are mentioned in the same story. Only extract data about the target monster.
When a monster’s behavior implies a strategic need (e.g., “each wound makes it stronger”), infer that the optimal tower speed is Fast to prevent escalation.
If an attribute is described as the only way to overcome a mechanic (e.g., “shield could only be broken by fire”), assign it to special_eff even if it also appears in weak.
Output ONLY the JSON object. Do not add any other text before or after.
Example Input:
...
Monster Name: the Iron Colossus
Number of stories: 1
Story 1:
...
response = client.chat.completions.create(
    model="/mnt/models/lora-1111", # 这个模型名称可以是您启动时使用的路径或任意名称，如果未指定 --served-model-name
    messages=[
